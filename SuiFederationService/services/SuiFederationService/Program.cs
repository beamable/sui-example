...
namespace Beamable.SuiFederationService
{
	...
	{
		/// <summary>
		/// The entry point for the <see cref="SuiFederationService"/> service.
		/// </summary>
		public static async Task Main()
		{
			// inject data from the CLI
			await MicroserviceBootstrapper.Prepare<SuiFederationService>();
			
			// load environment variables from local file
			LoadEnvironmentVariables();
			
			// run the Microservice code
			await MicroserviceBootstrapper.Start<SuiFederationService>();
		}
