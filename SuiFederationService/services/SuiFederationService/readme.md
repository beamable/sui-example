...
/project
  ...
  /services
     /SuiFederationService
        SuiFederationService.csproj
        Dockerfile
        Program.cs
        SuiFederationService.cs
     /SuiFederationServiceLibrary
        SuiFederationServiceLibrary.csproj
        SuiFederationServiceLibrary.cs
```

## Running Locally
The SuiFederationService project can be run locally by starting it from an IDE, or by executing the `dotnet run` command.
...
```shell
beam services deploy --ids SuiFederationService
```

### MSBuild Properties
The `SuiFederationService.csproj` file has a few configuration options that can be set.
```xml
<PropertyGroup>
...
#### BeamableTool
By default, both SuiFederationService and SuiFederationServiceLibrary have the BeamCLI installed as a project tool. 
...
remote swagger. 
```shell
beam project open-swagger SuiFederationService
```
...
at each linked Unity project's `Assets/Beamable/Microservice/Microservices/SuiFederationServiceClient.cs`. 

Additionally, the `share-code` msbuild target in the SuiFederationServiceLibrary project will copy their
